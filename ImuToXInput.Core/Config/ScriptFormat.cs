...
namespace ImuToXInput.Config
{
    public static class ScriptFormat
    {
        public static string FormatProfile(GameProfile profile)
        {
            ...
            if (profile.MenuModeToggleCondition != null)
            {
                sb.AppendLine("// Menu mode toggle: when condition becomes true, menu mode is toggled (no controller output).");
                sb.AppendLine("menuModeToggle " + FormatCondition("when", profile.MenuModeToggleCondition));
            }
            return sb.ToString();
        }
        ...
        private static GameProfile ParseProfile(string script, List<string>? errors)
        {
            ...
            foreach (var raw in lines)
            {
                ...
                if (line.StartsWith("button ", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        errors?.Add($"Line {lineNum}: invalid button syntax (expected: when <expr> -> Button)");
                    }
                    continue;
                }
                                ...
                                if (parts.Length >= 1)
                                {
                                    ...
                                    if (dot >= 0)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(tracker) && !string.IsNullOrEmpty(source))
                                        {
                                            ...
                                            {
                                                if (parts[i].Equals("invert", StringComparison.OrdinalIgnoreCase))
                                                {
                                                    axis.Invert = true;
                                                }
                                                else if (parts[i] == "*" && i + 1 < parts.Length && float.TryParse(parts[i + 1], System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out float sc))
                                                {
                                                    axis.Scale = sc;
                                                    i++;
                                                }
                ...
                if (line.StartsWith("menuModeToggle ", StringComparison.OrdinalIgnoreCase))
                {
                    var rest = line.Substring(14).Trim();
                    if (rest.StartsWith("when ", StringComparison.OrdinalIgnoreCase))
                    {
                        var expr = rest.Substring(5).Trim();
                        var cond = ParseCondition(expr);
                        if (cond != null)
                        {
                            profile.MenuModeToggleCondition = cond;
                        }
                        else
                            errors?.Add($"Line {lineNum}: invalid menuModeToggle condition");
                    }
                    else
                    {
                        errors?.Add($"Line {lineNum}: menuModeToggle expects 'when <expr>'");
                    }
                    continue;
                }
            }
